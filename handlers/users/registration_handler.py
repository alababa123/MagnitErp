...
async def reg_check(call: CallbackQuery, state: FSMContext):
    ...
    if callback_data == "reg:True":
        ...
        cur.execute("INSERT INTO tabEmployer (name ,creation ,owner ,fio ,phone_number ,telegramid ,photo_pass ,photo, status, telegramidforeman, amounttask, amountjob, rate, amounttask_now) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
