...
#        cur.execute("select fio, phone_number from tabEmployer where name=?", [data[0][0]])
...
#                    [day[0][2]])
#        conn.commit()
#        await bot.answer_callback_query(call.id, text="Срок изменен", show_alert=False)
#        btn = []
...
#                               text="🕑 Инженер одобрил вашу просьбу по задаче %s, срок изменен." % exp_end[0][1], reply_markup=bnt_inl)
#        conn.commit()
#        cur.execute("select name, task from tabshift_deadlines where foreman=? and status=?",
#                    [call.from_user.id, "На рассмотрении"])
#        task_name = cur.fetchall()
...
#        cur.execute("select task from tabshift_deadlines where name=?", [mas[2]])
#        day = cur.fetchall()
...
#        conn.commit()
#        await bot.answer_callback_query(call.id, text="Готово!", show_alert=False)
#        conn.commit()
#        cur.execute("select name, task from tabshift_deadlines where foreman=? and status=?",
#                    [call.from_user.id, "На рассмотрении"])
#        task_name = cur.fetchall()
...
#    cur.execute("select subject, workerid from tabTask where name=?", [data.get("task_name")])
#    subj = cur.fetchall()
...
#        inline_keyboard=btn,
#    )
#    await bot.send_message(subj[0][1], "Инженер отклонил вашу просьбу по задаче %s."
#                                               "\n➖➖➖➖➖➖➖➖➖➖➖\n"
#                                               "Комментарий: %s" % (subj[0][0], mes), reply_markup=bnt_inl)
...
#    conn.commit()
#    cur.execute("select fio, telegramid from tabEmployer where telegramidforeman='' and role='Рабочий' and status='Подтвержден'")
#    a = cur.fetchall()
#    free_work = []
#    for i in a:
...
#                    " from tabEmployer where role='Рабочий' and status='Подтвержден' and telegramid=%s" % str)
#        a = cur.fetchall()
#        free_work = []
...
#        conn.commit()
#        await bot.answer_callback_query(call.id, "Рабочий %s был переведён к вам!" %data.get("fio"), show_alert=False)
#        cur.execute("select fio, telegramid from tabEmployer where telegramidforeman='' and role='Рабочий' and status='Подтвержден'")
#        a = cur.fetchall()
#        free_work = []
#        for i in a:
...
async def work(call: CallbackQuery, state=FSMContext):
    ...
    cur = conn.cursor()
    conn.commit()
    cur.execute("select fio, telegramid, activity from tabEmployer where status='Подтвержден' and role='Рабочий' and telegramidforeman=%s" %call.from_user.id)
    a = cur.fetchall()
    free_work = []
    for i in a:
        if (i[2] == 0):
...
async def free_work(call: CallbackQuery, state=FSMContext):
    ...
    if (str == "Назад"):
        ...
        cur.execute("select fio, phone_number, telegramid, comments_foreman, photo, photo_pass, amounttask, amounttask_month, amounttask_cancel, "
                    "amounttask_now, amounttask_cancel_now, amounttask_job"
            " from tabEmployer where role='Рабочий' and status='Подтвержден' and telegramid=%s" % str)
        a = cur.fetchall()
        free_work = []
#        free_work.append([InlineKeyboardButton(text="Удалить", callback_data="Удалить")])
        ...
        await call.message.edit_text("Профиль рабочего %s\n"
                                ...
                                "Просроченные: %s\n"
                                "Назначенные: %s"
                                "\n➖➖➖➖➖➖➖➖➖➖➖\n" %(a[0][0], a[0][1], a[0][6], a[0][7], a[0][10], a[0][11] , a[0][8], a[0][9]),
                                     reply_markup=foreman_choise_free_btn)
        await state.update_data(fio=a[0][0], phone_number=a[0][1], telegramid=a[0][2], comment=a[0][3], photo=a[0][4], passport=a[0][5])
        conn.close()
...
async def invite_team(call: CallbackQuery, state=FSMContext):
    ...
    if (str == "Назад"):
        conn.commit()
        cur.execute(
            "select fio, telegramid, activity from tabEmployer where status='Подтвержден' and role='Рабочий' and telegramidforeman=%s" % call.from_user.id)
        a = cur.fetchall()
        free_work = []
        for i in a:
            if (i[2] == 0):
    ...
    #    conn.commit()
    #    await bot.answer_callback_query(call.id, text="Рабочий %s был перенесён в архив!" %data.get("fio"), show_alert=False)
    #    conn.commit()
    #    cur.execute(
    #        "select fio, telegramid, activity from tabEmployer where status='Подтвержден' and role='Рабочий' and telegramidforeman=%s" % call.from_user.id)
    #    a = cur.fetchall()
...
async def choise_overdue(call: CallbackQuery, state=FSMContext):
    ...
    if(call.data == "Назад"):
        ...
        conn.commit()
        cur.execute("select workerid, subject, date_progress, date_perfomance, exp_start_date, exp_end_date from tabTask where name=?", [call.data])
        task = cur.fetchall()
        if(task):
            ...
            if(fio):
                ...
                free_work.append([InlineKeyboardButton(text="Назад", callback_data="Назад")])
                foreman_btn = InlineKeyboardMarkup(inline_keyboard=free_work, )
                await call.message.edit_text("❗ Внимание ❗ \n"
                                             "❗ Задача %s просрочена ❗\n"
                                             "❗ Рабочий не указан ❗"
                                             "\n➖➖➖➖➖➖➖➖➖➖➖\n"
                                             " Сроки задачи: %s ➡️%s " %(
                                             task[0][1], task[0][4], task[0][5]), reply_markup=foreman_btn)
                conn.close()
                await foreman.overdue_down.set()
        else:
            await call.message.answer("Бот не понял запрос, вы будете возвращены в главное меню.", reply_markup=foreman_menu)
            conn.close()
            await foreman.job.set()
...
async def work(call: CallbackQuery, state=FSMContext):
    ...
    if(call_data == "Назад"):
        ...
        cur.execute("select name, subject, subdivision from tabTask where workerid=? and status='Report'", [call_data])
        a = cur.fetchall()
        cur.execute("select fio from tabEmployer where name=?", [call_data])
        b = cur.fetchall()
        free_work = []
        for i in a:
            free_work.append([InlineKeyboardButton(text=i[2] , callback_data=i[0] + "+" + call_data)])
        free_work.append([InlineKeyboardButton(text="Назад", callback_data="Назад")])
        foreman_btn = InlineKeyboardMarkup(inline_keyboard=free_work,)
...
async def free_work(call: CallbackQuery, state=FSMContext):
    ...
    if (call_data == "Назад"):
        ...
        mas = call_data.split("+")
        cur.execute("select name, subject, amount, date, subdivision, description, exp_start_date, exp_end_date from tabTask where name=? and workerid=?", mas)
        a = cur.fetchall()
        ...
        if(k == 2):
            media_id = await bot.send_media_group(call.from_user.id, media=media)
            media_id_message = []
            for i in media_id:
                print(i.message_id)
                media_id_message.append(i.message_id)
            await state.update_data(media_message_id=media_id_message)
        if(len(a[0][5]) > 3500 and len(a[0][5]) < 4096):
            await call.message.answer(  "Подразделение: %s"
                                        "\n➖➖➖➖➖➖➖➖➖➖➖\n"
                                        "Дата начала: %s\n"
                                        "Дата окончания: %s"
                                        "\n➖➖➖➖➖➖➖➖➖➖➖\n"
                                        "Текст отчёта: %s\n"
                                        "Номер телефона рабочего: %s\n"
                                        "Имя рабочего: %s" %(a[0][4], a[0][6], a[0][7], a[0][2], b[0][1], b[0][0]))
            mes = await call.message.answer(f"Подробности задачи: {a[0][5]}", reply_markup=foreman_choise_free_btn)
            await state.update_data(second_message_id=mes.message_id - 1)
        elif(len(a[0][5]) >= 4096):
            await call.message.answer("Подразделение: %s"
                                      "\n➖➖➖➖➖➖➖➖➖➖➖\n"
                                      ...
                                      "Текст отчёта: %s\n"
                                      "Номер телефона рабочего: %s\n"
                                      "Имя рабочего: %s" % (a[0][4], a[0][6], a[0][7], a[0][2], b[0][1], b[0][0]),reply_markup=foreman_choise_free_btn)
        else:
            await call.message.answer("Подразделение: %s"
                                      "\n➖➖➖➖➖➖➖➖➖➖➖\n"
                                      ...
                                      "Текст отчёта: %s\n"
                                      "Номер телефона рабочего: %s\n"
                                      "Имя рабочего: %s" % (a[0][4], a[0][5], a[0][6], a[0][7], a[0][2], b[0][1], b[0][0]),
                                      reply_markup=foreman_choise_free_btn)
...
async def invite_team(call: CallbackQuery, state=FSMContext):
    ...
    if (call_data == "Назад"):
        ...
        if (second_message_id):
            await bot.delete_message(call.from_user.id, second_message_id)
            await state.update_data(second_message_id='')
        media_group_id = data.get("media_message_id")
        if(media_group_id):
            for i in media_group_id:
                await bot.delete_message(call.from_user.id, i)
            await state.update_data(media_message_id=[])
        cur.execute("select name, subject, subdivision from tabTask where workerid=? and status='Report'", [data.get("telegramid_report")])
        a = cur.fetchall()
        cur.execute("select fio from tabEmployer where name=?", [data.get("telegramid_report")])
        b = cur.fetchall()
        free_work = []
        for i in a:
            free_work.append([InlineKeyboardButton(text=i[2], callback_data=i[0] + "+" + data.get("telegramid_report"))])
        free_work.append([InlineKeyboardButton(text="Назад", callback_data="Назад")])
        foreman_btn = InlineKeyboardMarkup(inline_keyboard=free_work, )
        await call.message.edit_text(text="Список отчетов рабочего %s" % b[0][0], reply_markup=foreman_btn)
        conn.close()
        ...
        progress = float(100)
        cur.execute("update tabTask set modified=?, progress=?, status='Completed', date_complete=? where name=? and workerid=?", [datetime.datetime.now(), float(progress), datetime.datetime.now().date().strftime("%Y-%m-%d"),  data.get("task_name"), data.get("telegramid_report")])
        conn.commit()
        await bot.answer_callback_query(call.id, text="Готово! Отчет занесён в базу!", show_alert=False)
        ...
        cur.execute("update tabEmployer set amounttask=?, rate=? where name=?", [amount_task, rate, data.get("telegramid_report")])
        conn.commit()
        cur.execute("select name, subject, subdivision from tabTask where workerid=? and status='Report'", [tempa[0]])
        a = cur.fetchall()
        cur.execute("select fio from tabEmployer where name=?", [tempa[0]])
        b = cur.fetchall()
        free_work = []
        for i in a:
            free_work.append([InlineKeyboardButton(text=i[2], callback_data=i[0] + "+" + tempa[0])])
        free_work.append([InlineKeyboardButton(text="Назад", callback_data="Назад")])
        foreman_btn = InlineKeyboardMarkup(inline_keyboard=free_work, )
        await call.message.edit_text(text="Список отчетов рабочего %s" % b[0][0], reply_markup=foreman_btn)
        conn.close()
        ...
        free_work.append([InlineKeyboardButton(text="Назад", callback_data="Назад")])
        foreman_btn = InlineKeyboardMarkup(inline_keyboard=free_work, )
        mesag = await call.message.answer("Пожалуйста, напишите причину отклонения отчёта. \nВы можете нажать `Отклонить отчёт без комментария`, чтобы не писать комментарий.\n",
            reply_markup=foreman_btn)
        await state.update_data(cancel_report_bnt=mesag.message_id)
        conn.close()
...
async def back_to_profile(call: CallbackQuery, state=FSMContext):
    ...
    if(call_back == "Назад"):
        data = await state.get_data()
        print(data)
        cur.execute("select name, subject, subdivision from tabTask where workerid=? and status='Report'",
        ...
        if (history_mas and history_mas[0][0] != None):
            ...
            history = '[' + datetime.datetime.now().strftime("'%Y-%m-%d %H:%M:%S") + ' ' + name[0][0] + '] ' + "Причина не указана"
        cur.execute("update tabTask set status='Cancelled', modified=?, history=? where workerid=? and name=?", [datetime.datetime.now(), history, data.get("telegramid_report"), data.get("task_name")])
        conn.commit()
        await bot.answer_callback_query(call.id, text="Готово! Отчет отклонён!", show_alert=False)
        cur.execute("select name, subject, subdivision from tabTask where workerid=? and status='Report'",
                    [data.get("telegramid_report")])
        a = cur.fetchall()
        cur.execute("select fio from tabEmployer where name=?", [data.get("telegramid_report")])
        ...
        for i in a:
            free_work.append(
                [InlineKeyboardButton(text=i[2], callback_data=i[0] + "+" + data.get("telegramid_report"))])
        free_work.append([InlineKeyboardButton(text="Назад", callback_data="Назад")])
        foreman_btn = InlineKeyboardMarkup(inline_keyboard=free_work, )
        await call.message.edit_text(text="Список отчетов рабочего %s" % b[0][0], reply_markup=foreman_btn)
        conn.close()
...
async def cancel_report(message: Message, state=FSMContext):
    ...
    data = await state.get_data()
    mes = message.text
    message_id = data.get("cancel_report_bnt")
    ...
    cur.execute("update tabTask set modified=?, comment_foreman=?, status='Cancelled', amount='', history=? where workerid=? and name=?", [datetime.datetime.now(), comment_answer, history, temp[0], temp[1]])
    conn.commit()
