    ...
    if(a):
        if (a[0][1]=='Подтвержден'):
            if(a[0][2]=='ROLE-0001'):
                ...
                obj = cur.fetchall()
                if (obj[0][0]):
                    await message.answer('Вас приветствует персональный помощник "Цифрум" "! \nНажмите кнопку "Начать рабочий день", чтобы начать работу.\n\n⚠️ Тех.поддержка https://t.me/barmem\n\n📞 Телефон тех. поддержки +79998681211 (Игорь)', disable_web_page_preview=True, reply_markup=foreman_start_job)
                else:
                    await message.answer('Добрый день! Вы не прикреплены к объекту.', reply_markup=foreman_start_job)
            elif(a[0][2]=='ROLE-0002'):
                if(a[0][3] != None):
                    await message.answer('Вас приветствует персональный помощник "Цифрум" "! \nНажмите кнопку "Начать рабочий день", чтобы начать работу.\n\n⚠️ Тех.поддержка https://t.me/barmem\n\n📞 Телефон тех. поддержки +79998681211 (Игорь)', disable_web_page_preview=True, reply_markup=worker_start_job)
                else:
                    await message.answer(f"Добрый день, у вас нет руководителя, нажмите на кнопку, чтобы проверить обновления.\n\n⚠️ Тех.поддержка https://t.me/barmem\n\n📞 Телефон тех. поддержки +79998681211 (Игорь)", disable_web_page_preview=True, reply_markup=worker_no_job)
                    await worker.no_job.set()
            elif(a[0][2]):
                await message.answer("Добрый день, для вашей должности пока нет функционала.\n\n⚠️ Тех.поддержка https://t.me/barmem\n\n📞 Телефон тех. поддержки +79998681211 (Игорь)", disable_web_page_preview=True, reply_markup=ReplyKeyboardRemove())
            else:
        ...
        await message.answer('Нажмите кнопку "Зарегистрироваться", чтобы пройти регистрацию.\n\nПосле подтверждения личности, вы можете начать работу!\n\n⚠️ Связаться с тех.поддержкой https://t.me/barmem \n\n📞 Либо позвоните по телефону +79998681211 (Игорь)', disable_web_page_preview=True,  reply_markup=reg)
